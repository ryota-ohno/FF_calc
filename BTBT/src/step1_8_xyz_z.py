...
from make_8_xyz_z import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['theta','a','b','z','E','E1','E2','E3','E4','status'])##いじる
    ...
    if not os.path.exists(auto_csv_path2):        
        df_E_2 = pd.DataFrame(columns = ['theta','b','z','E2','machine_type','status','file_name'])##いじる
    ...
    if not os.path.exists(auto_csv_path3):        
        df_E_3 = pd.DataFrame(columns = ['theta','a','b','z','E3','E4','machine_type','status','file_name'])##いじる
    ...
    while not(isOver):
        #check
        isOver = listen(args.auto_dir,args.monomer_name,args.num_nodes,args.max_nodes,args.max_2,args.isTest)##argsの中身を取る
        time.sleep(1)

def listen(auto_dir,monomer_name,num_nodes,max_nodes,max_2,isTest):##args自体を引数に取るか中身をばらして取るかの違い
